...
const PELIGRO_COLORS={Procesionaria:'#D91B1D',Garrapatas:'#8e44ad',Cristales:'#e67e22',Veneno:'#2c3e50',Otro:'#7f8c8d'};
function getPeligroIcon(tipo){return PELIGRO_ICONS[tipo]||'⚠️';}
function getPeligroTablerIcon(tipo){const map={'Procesionaria':'ti-line-dashed','Garrapatas':'ti-bug','Cristales':'ti-triangles','Veneno':'ti-skull','Otro':'ti-alert-triangle'};return map[tipo]||'ti-alert-triangle';}
function getPeligroColor(tipo){return PELIGRO_COLORS[tipo]||'#D91B1D';}
...
function getHighestBadgeSocial(d,r){const u=BADGES_SOCIALES.filter(b=>b.check(d||0,r||0));return u.length>0?u[u.length-1]:BADGES_SOCIALES[0];}
const RING_CLASS_BY_BADGE={nuevo_manada:'ring-nuevo',primera_huellita:'ring-primera',patas_sol:'ring-patas',hijo_isla:'ring-hijo',alma_isla:'ring-alma'};
...
async function openMisReportes(){
  ...
  try {
    ...
    if(!Array.isArray(data) || data.length === 0){
      container.innerHTML = '<div class="empty-state"><span class="empty-state-emoji">🌿</span><div class="empty-state-text">Tu zona está tranquila, ¡bien por ti! Cuando veas algo, repórtalo desde el mapa.</div></div>';
      return;
    }
...
function initMap(){
  ...
  window.rIcon=createRiskIcon('#D91B1D');
  ...
  vets.forEach(v=>{L.marker([v.lat,v.lng],{icon:bIcon}).addTo(window.map).bindPopup(`<div style="font-size:13px">🏥 <b>${v.name}</b><br><small>${v.addr}</small><br><br><a href="tel:${v.phone}" style="display:block;margin-bottom:6px;background:#D91B1D;color:#fff;padding:6px;border-radius:6px;text-align:center;text-decoration:none">📞 Llamar</a><a href="${v.maps}" target="_blank" style="display:block;background:#3498db;color:#fff;padding:6px;border-radius:6px;text-align:center;text-decoration:none">🧭 Cómo llegar</a></div>`);});
  loadAvistamientos();
}
...
async function loadAvistamientos(){
  ...
  data.forEach(a=>{
    ...
    const icon=createRiskIcon('#D91B1D',isHistorico(a.created_at));
    const tipo=a.tipo_peligro||'Procesionaria';
    const popupHtml=`<div class="popup-clickable" onclick="goToCard('${a.id}','.avist-item')"><div style="font-size:13px"><i class="ti ${getPeligroTablerIcon(tipo)}"></i> <b>${escapeHtml(a.ubicacion||'Sin ubicación')}</b><br><span style="color:#666;font-size:11px">${timeAgo(a.created_at)}</span><br>${escapeHtml(tipo)} · ${escapeHtml(a.riesgo||'')}</div><span class="popup-hint"><i class="ti ti-eye"></i> Ver detalles ↓</span></div>`;
    const mk=L.marker([lat,lng],{icon}).addTo(markersLayer).bindPopup(popupHtml);
    markersById[a.id]=mk;
  });
}
  const container=document.getElementById('avist-container');if(data.length===0){container.innerHTML='<div class="empty-state"><span class="empty-state-emoji"><i class="ti ti-mood-smile"></i></span><div class="empty-state-text">Todo tranquilo por aquí. Te avisaremos si algo cambia.</div></div>';return;}
  container.innerHTML=data.map(a=>{const riesgo=a.riesgo||'Sin especificar';const clase=riesgo.toLowerCase().includes('medio')?'medio':riesgo.toLowerCase().includes('bajo')?'bajo':'alto';const hist=isHistorico(a.created_at);const conf=a.confirmations||0;const ya=localStorage.getItem('pdi_conf_'+a.id);const nameInfo=a.reporter_id?namesCache[a.reporter_id]:null;const reporterName=nameInfo?nameInfo.name:'';const rCount=a.reporter_id?(reportCounts[a.reporter_id]||0):0;const topBadge=getHighestBadge(rCount,0);const badgeTag=topBadge?` · ${topBadge.emoji} ${topBadge.name}`:'';const tipo=a.tipo_peligro||'Procesionaria';const tipoLabel=tipo==='Otro'&&a.otro_peligro?a.otro_peligro:tipo;return `<div class="avist-item ${hist?'historico':''}" data-id="${a.id}"><div class="avist-icon"><i class="ti ${getPeligroTablerIcon(tipo)}"></i></div><div class="avist-info"><h4 class="clickable-location" onclick="goToMapMarker('${a.id}','${a.lat}','${a.lng}')">${escapeHtml(a.ubicacion||'Ubicación desconocida')}</h4><div style="font-size:11px;font-weight:700;color:${getPeligroColor(tipo)};margin:3px 0;text-transform:uppercase;letter-spacing:.5px"><i class="ti ${getPeligroTablerIcon(tipo)}"></i> ${escapeHtml(tipoLabel)}</div>${(()=>{const arr=(a.fotos&&Array.isArray(a.fotos)&&a.fotos.length>0)?a.fotos:(a.foto?[a.foto]:[]);if(arr.length===0) return '';if(arr.length===1) return `<img src="${arr[0]}" alt="Alerta" onclick="openImage('${arr[0]}')" class="avist-img">`;return `<div class="gallery-scroll">${arr.map(u=>`<img src="${u}" onclick="openImage('${u}')" class="gallery-img">`).join('')}</div><div class="gallery-dots">${arr.length} fotos · desliza →</div>`;})()}<p>${escapeHtml(a.descripcion||'Sin descripción')}</p><div class="avist-meta"><span><i class="ti ti-clock"></i> ${timeAgo(a.created_at)}</span>${a.edited_at?'<span class="edited-mark">(editado)</span>':''}${a.distance?`<span class="dot-sep">·</span><span><i class="ti ti-map-pin"></i> A ${a.distance.toFixed(1)} km</span>`:''}<span class="dot-sep">·</span><span class="badge ${clase}">${riesgo}</span>${hist?'<span class="badge historico">Histórico</span>':''}</div>${reporterName?`<p class="avist-reporter">${nameInfo&&nameInfo.foto?`<img src="${nameInfo.foto}" onclick="event.stopPropagation();${a.reporter_id!==USER_ID?`abrirMiniPerfil('${a.reporter_id}')`:`openImage('${nameInfo.foto}')`}" class="clickable-img" style="width:20px;height:20px;border-radius:50%;object-fit:cover;vertical-align:middle;margin-right:4px">`:''}Reportado por ${escapeHtml(reporterName)}${badgeTag}</p>`:''}${conf>0?`<p class="confirm-count"><i class="ti ti-circle-check"></i> ${conf} persona${conf>1?'s':''} confirmó que sigue ahí</p>`:''}<div class="confirm-actions"><button class="btn-confirm" onclick="confirmSighting('${a.id}',true)" ${ya?'disabled':''}>${ya==='confirm'?'<i class="ti ti-circle-check"></i>CONFIRMADO':'<i class="ti ti-eye"></i>SIGUE AHÍ'}</button><button class="btn-deny" onclick="confirmSighting('${a.id}',false)" ${ya?'disabled':''}>${ya==='deny'?'<i class="ti ti-x"></i>DESMENTIDO':'<i class="ti ti-circle-x"></i>YA NO ESTÁ'}</button></div>${a.reporter_id===USER_ID?`<button class="btn-edit-own" onclick='openEditModal(${JSON.stringify(a).replace(/'/g,"&#39;").replace(/"/g,"&quot;")})'><i class="ti ti-pencil"></i>EDITAR MI REPORTE</button>`:''}<button class="btn-huellita" onclick="darHuellita('reporte','${a.id}',this)">🐾 <span class="huellita-count">0</span></button></div></div>`;}).join('');checkHotZone();
...
function renderRutasOnMap(rutas){
  ...
  cachedAvistamientos.forEach(a=>{
    const lat=parseFloat(a.lat),lng=parseFloat(a.lng);
    if(isNaN(lat)||isNaN(lng)) return;
    const icon=createRiskIcon('#D91B1D',true);
    const tipo=a.tipo_peligro||'Procesionaria';
    const tipoIcon=getPeligroIcon(tipo);
    const tipoLabel=tipo==='Otro'&&a.otro_peligro?a.otro_peligro:tipo;
    const riesgo=a.riesgo||'';
    const popupHtml=`<div style="font-size:12px"><i class="ti ${getPeligroTablerIcon(tipo)}"></i> <b>${escapeHtml(tipoLabel)}</b>${riesgo?' · '+escapeHtml(riesgo):''}<br>${escapeHtml(a.ubicacion||'')}<br><span style="color:#888;font-size:11px">${timeAgo(a.created_at)}</span></div>`;
  ...
  rutas.forEach(r=>{
    ...
    if(wps&&wps.length>=2){
      ...
      const startPopup=`<div class="popup-clickable" onclick="goToCard('${r.id}','.ruta-card')"><div style="font-size:13px">${isVer?'<i class="ti ti-circle-check"></i>':'<i class="ti ti-paw"></i>'} <b>${escapeHtml(r.nombre)}</b><br><span style="color:#27ae60;font-size:11px">INICIO</span>${isVer?'<br><span style="color:#27ae60;font-size:11px;font-weight:700">VERIFICADA</span>':''}</div><span class="popup-hint"><i class="ti ti-eye"></i> Ver detalles ↓</span></div>`;
      ...
      L.marker([lastWp.lat,lastWp.lng],{icon:L.divIcon({html:'<div class="wp-end">B</div>',iconSize:[22,22],iconAnchor:[11,11],className:''})}).addTo(rutasLayer).bindPopup(`<div style="font-size:13px">${isVer?'<i class="ti ti-circle-check"></i>':'<i class="ti ti-paw"></i>'} <b>${escapeHtml(r.nombre)}</b><br><span style="color:#D91B1D;font-size:11px">FINAL</span></div>`);
    } else {
      const lat=parseFloat(r.lat),lng=parseFloat(r.lng);
      if(isNaN(lat)||isNaN(lng)) return;
      const singlePopup=`<div class="popup-clickable" onclick="goToCard('${r.id}','.ruta-card')"><div style="font-size:13px">${isVer?'<i class="ti ti-circle-check"></i>':'<i class="ti ti-paw"></i>'} <b>${escapeHtml(r.nombre)}</b><br><span style="color:#888;font-size:11px">${escapeHtml(r.distancia||'')} · ${escapeHtml(r.dificultad||'')}</span>${isVer?'<br><span style="color:#27ae60;font-size:11px;font-weight:700">VERIFICADA</span>':''}</div><span class="popup-hint"><i class="ti ti-eye"></i> Ver detalles ↓</span></div>`;
...
function renderRutasList(rutas){const container=document.getElementById('rutas-container');if(rutas.length===0){container.innerHTML='<div class="empty-state"><span class="empty-state-emoji"><i class="ti ti-shoe"></i></span><div class="empty-state-text">¡La aventura espera! Sé el primero en compartir una ruta segura.</div></div>';return;}
container.innerHTML=rutas.map(r=>{const nearby=getNearbyAlerts(r.lat,r.lng,1.5,r.waypoints);const nameInfo=r.reporter_id?namesCache[r.reporter_id]:null;const reporterName=nameInfo?nameInfo.name:'';const mapsUrl=r.waypoints&&r.waypoints.length>=2?`https://www.google.com/maps/dir/${r.waypoints.map(w=>w.lat+','+w.lng).join('/')}`:`https://maps.google.com/?q=${r.lat},${r.lng}`;const wpDist=calcWaypointsDistance(r.waypoints);let distHtml='';if(wpDist!==null){distHtml=`<div class="ruta-distance-calc"><i class="ti ti-ruler"></i> Distancia real: <strong>${wpDist<1?Math.round(wpDist*1000)+' m':wpDist.toFixed(1)+' km'}</strong> · ${r.waypoints.length} puntos</div>`;}let tags='';if(r.distancia) tags+=`<span class="ruta-tag"><i class="ti ti-ruler-2"></i>${escapeHtml(r.distancia)}</span>`;if(r.dificultad) tags+=`<span class="ruta-tag"><i class="ti ti-mountain"></i>${escapeHtml(r.dificultad)}</span>`;if(r.suelo) tags+=`<span class="ruta-tag"><i class="ti ti-shoe"></i>${escapeHtml(r.suelo)}</span>`;if(r.sombra) tags+=`<span class="ruta-tag"><i class="ti ti-tree"></i>Sombra: ${escapeHtml(r.sombra)}</span>`;if(r.agua&&r.agua!=='No') tags+=`<span class="ruta-tag"><i class="ti ti-droplet"></i>${escapeHtml(r.agua)}</span>`;if(r.tipo_perro&&r.tipo_perro!=='Todos') tags+=`<span class="ruta-tag"><i class="ti ti-paw"></i>${escapeHtml(r.tipo_perro)}</span>`;let alertasHtml='';if(nearby.length>0){alertasHtml=`<div class="ruta-alertas"><div class="ruta-alertas-title"><i class="ti ti-alert-triangle"></i>${nearby.length} alerta${nearby.length>1?'s':''} de peligro cerca</div>${nearby.slice(0,3).map(a=>`<div class="ruta-alerta-item"><i class="ti ${getPeligroTablerIcon(a.tipo_peligro||'Procesionaria')}"></i>${escapeHtml(a.ubicacion||'Zona cercana')} — ${escapeHtml(a.tipo_peligro||'Procesionaria')} · ${timeAgo(a.created_at)}</div>`).join('')}${nearby.length>3?`<div class="ruta-alerta-item" style="color:#888">...y ${nearby.length-3} más</div>`:''}</div>`;}else{alertasHtml='<div class="ruta-safe"><i class="ti ti-circle-check"></i>Sin alertas de peligro recientes en esta zona</div>';}let notasHtml='';if(r.notas_carlos){notasHtml=`<div class="ruta-notas-carlos"><div class="ruta-notas-carlos-title">🐾 Nota de Perros de la Isla</div><p>${escapeHtml(r.notas_carlos)}</p></div>`;}const sello=r.verificada?`<span class="sello-verificada"><i class="ti ti-circle-check-filled"></i>Verificada</span>`:`<span class="sello-pendiente"><i class="ti ti-clock"></i>Pendiente</span>`;return `<div class="ruta-card" data-id="${r.id}">${r.foto?`<img src="${r.foto}" alt="${escapeHtml(r.nombre)}" class="ruta-card-img" onclick="openImage('${r.foto}')">`:''}<div class="ruta-card-body"><div class="ruta-card-header"><h4 class="clickable-location" onclick="goToMapMarker('${r.id}','${(r.waypoints&&r.waypoints[0])?r.waypoints[0].lat:r.lat}','${(r.waypoints&&r.waypoints[0])?r.waypoints[0].lng:r.lng}')">${escapeHtml(r.nombre)}</h4>${sello}</div><p class="ruta-desc">${escapeHtml(r.descripcion||'Sin descripción')}</p>${distHtml}<div class="ruta-tags">${tags}</div>${alertasHtml}${notasHtml}<div class="ruta-meta"><span><i class="ti ti-clock"></i> ${timeAgo(r.created_at)}</span>${reporterName?`<span class="dot-sep">·</span><span>Sugerida por ${escapeHtml(reporterName)}</span>`:''}</div><div class="ruta-actions"><button class="btn-ruta-nav" onclick="window.open('${mapsUrl}')"><i class="ti ti-compass"></i>CÓMO LLEGAR</button></div></div></div>`;}).join('');}
...
async function openRanking(){
  ...
  try{
    ...
    if(ranking.length===0){container.innerHTML='<div class="empty-state"><span class="empty-state-emoji">🏆</span><div class="empty-state-text">Aún no hay pisadas en el ranking. ¡Sé tú la primera!</div></div>';return;}
...
async function darHuellita(targetType, targetId, btnEl) {
  ...
  // Microinteracción inmediata (no espera al servidor)
  btnEl.classList.remove('huellita-pulse');
  void btnEl.offsetWidth;
  btnEl.classList.add('huellita-pulse');
  setTimeout(()=>btnEl.classList.remove('huellita-pulse'), 320);
...
async function loadInicio(){
  ...
  if(fotoEl){
    ...
    // Anillo de progreso según badge social actual
    Object.values(RING_CLASS_BY_BADGE).forEach(c=>fotoEl.classList.remove(c));
    fotoEl.classList.add(RING_CLASS_BY_BADGE.nuevo_manada);
    if(profile){
      try{
        const ru=await fetch(SUPA_URL+`/rest/v1/usuarios?id=eq.${USER_ID}&select=huellitas_dadas,huellitas_recibidas`,{headers:HEADERS});
        const[u]=await ru.json();
        const top=getHighestBadgeSocial(u?.huellitas_dadas,u?.huellitas_recibidas);
        const cls=RING_CLASS_BY_BADGE[top.id];
        if(cls){
          Object.values(RING_CLASS_BY_BADGE).forEach(c=>fotoEl.classList.remove(c));
          fotoEl.classList.add(cls);
        }
      }catch(e){}
    }
  }
...
async function loadIniBienvenida(){
  const card=document.getElementById('iniBienvenida');
  if(!card) return;
  const saludoEl=card.querySelector('.ini-bienv-saludo');
  const statEl=card.querySelector('.ini-bienv-stat');
  const profile=getProfile();
  const nombre=profile?.nombre?profile.nombre:'tutor';
  const h=new Date().getHours();
  const saludoTxt=(h>=5&&h<12)?'Buenos días':(h>=12&&h<20)?'Buenas tardes':'Buenas noches';
  if(saludoEl) saludoEl.textContent=`${saludoTxt}, ${nombre} 🐾`;
  if(statEl){
    statEl.textContent='';
    statEl.classList.remove('has-alerts');
    try{
      const since=new Date(Date.now()-86400000).toISOString();
      const r=await fetch(SUPA_URL+`/rest/v1/avistamientos?select=id&status=eq.activo&created_at=gte.${since}`,{headers:HEADERS});
      const d=await r.json();
      const n=Array.isArray(d)?d.length:0;
      if(n===0){
        statEl.textContent='Hoy todo tranquilo en Mallorca';
      }else{
        statEl.textContent=`Hoy hay ${n} alerta${n===1?'':'s'} activa${n===1?'':'s'} en Mallorca`;
        statEl.classList.add('has-alerts');
      }
    }catch(e){ statEl.textContent='Comunidad activa en Mallorca'; }
  }
